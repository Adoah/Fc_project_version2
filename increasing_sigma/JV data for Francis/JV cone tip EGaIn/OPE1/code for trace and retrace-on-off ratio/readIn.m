function [twotrace,onetrace] = readIn(dir,filename,plots)
    ...
    for i=1:L
        try % First try gaussian fit
            [mfj(i),sfj(i),cfj(:,i)] = gaussFit(vx(i),mj2(i,:)',bins,1); % current
            [mfc(i),sfc(i),cfc(:,i)] = gaussFit(vx(i),log10(abs(mc2(i,:)')),bins,1); % current density
        catch % Gaussian fit fails to converge, use mathematical
            try
                [mfj(i),sfj(i),cfj(:,i)] = gaussFit(vx(i),mj2(i,:)',bins+2,1); % current
                [mfc(i),sfc(i),cfc(:,i)] = gaussFit(vx(i),log10(abs(mc2(i,:)')),bins+2,1); % current density
            catch % Gaussian fit fails to converge, use mathematical
    ...
    %logmc = abs(log10( mc2(2:51,:)) - log10(fliplr(mc2(152:201,:))));
    logmc = real(log10( mc2(242:301,:) ./ fliplr(mc2(2:61,:)) ));
    logmc(isinf(logmc)|isnan(logmc))=0;
    obinsc = linspace(min(min(logmc)),max(max(logmc)),binNoc);

    for i=1:60
        [offonH,oobc]=histcounts(real(log10(mc2(302-i,:) ./ mc2(i+1,:)))',obinsc);
        offon(:,i) = offonH';
        % make on/off ratio average curves
        try
            [moo(i),soo(i),coo(:,i)] = gaussFit(vx(i),real(log10(mc2(302-i,:) ./ mc2(i+1,:))),obinsc,1); % current density
        catch % Gaussian fit fails to converge, use mathematical
            warning('Problem finding gaussian mean.  Computing arithmetic mean instead.');
            % arithmetic fit
            vx(i)
            [moo(i),soo(i),coo(:,i)] = arithFit(real(log10(mc2(302-i,:) ./ mc2(i+1,:))),runs); % current density
        end
    end

    %pcolor(v(2:51),obinsc(1:end-1),offon);
    %%% 2:51 / end-49:end ; 52:102 / end-99, end-49; 103:151 /
    %%% end-148:end-100; 152:201 / end-199:end-150
    
    for i=1:length(bc)-1
        bcc(i) = (bc(i) + bc(i+1)) / 2.;
    end
       for i=1:length(oobc)-1
        occ(i) = (oobc(i) + oobc(i+1)) / 2.;
    end
    
    %%% Export all data
    twotrace = {vx,mfj,sfj,cfj,d1fvx,d1fm,mfc,sfc,cfc,heatc,bcc,offon,occ,moo,soo,coo,vx(2:61)};
