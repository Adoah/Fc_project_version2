function [] = ivPlots(l,n)
    ...
    [v,mj,sj,cj,dx,dj,mc,sc,cc,hc,bc,oo,oobc,moo] = l{:}; % Extract all data from the list
    ...
    %plot background
    ylims = [-8 2]; % define limits -- change these to change limits
    bg = zeros(length(ylims),length(v)); % make matrix of zeros)
    pcolor(v,ylims,bg); % plot background

    hold on
    hcn = fliplr(hc(:,end/2:end)) + hc(:,1:end/2 +1);
    %pcolor(v(1:end/2+1),bc(2:end),hcn);
    pcolor(v(1:end/2+1),bc,hcn);
    %caxis([0 100]);
    shading interp;
    colormap jet;
    hold on
    plot(v,mc,'color','k','LineWidth',3);
    xlabel('voltage (V)', 'FontName', 'Arial', 'FontSize', 24);
    ylabel(['log_{10} |\it J \rm| (A/cm^2)'], 'FontName', 'Arial', 'FontSize', 24);
    set(gca, 'FontName', 'Arial', 'FontSize', fs);
    set(gcf, 'renderer', 'opengl');
    ...
    ylims = [0 6];
    bg = zeros(length(ylims),length(v(2:61))); % make matrix of zeros)
    pcolor(v(2:61),ylims,bg); % plot background
    hold on
    %offon = hc(:,2:51) ./ hc(:,152:201);
    %offon(isinf(offon)|isnan(offon))=0;
    %offon
    %pcolor(v(2:51),oobc(1:end-1),oo);
    pcolor(v(2:61),oobc,oo);
    %caxis([0 100]);
    shading interp;
    colormap jet;
    hold on
    plot(v(2:61),moo,'color','k','LineWidth',3);
    %plot(v(152:201),mc(152:201),'color','k');
    xlabel('voltage (V)', 'FontName', 'Arial', 'FontSize', 24);
    ylabel(['log_{10} (on/off ratio)'], 'FontName', 'Arial', 'FontSize', 24);
    set(gca, 'FontName', 'Arial', 'FontSize', fs);
    set(gcf, 'renderer', 'opengl');
    c = colorbar;
    xlim([-1.98 -0.8]);
